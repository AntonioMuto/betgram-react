      ...
      {fixture.events.length === 0 ? (
        ...
        <ul className="timeline timeline-vertical gap-4">
          {fixture.events.map((event, index) => {
            ...
            const icon = (
              <>
                ...
                {event.type === "Goal" && (
                  <Icon
                    iconNode={soccerBall}
                    className={`w-5 h-5 ${event.detail === "Own Goal" ? "text-red-500" : "text-white"
                      }`}
                  />
                )}
            ...
            const playerInfo = (
              <div
                className={`flex flex-col ${isSubstitution ? "text-red-400" : ""
            ...
            return (
              <li
                key={`${index}-${event.time.elapsed}-${event.player?.id}-${event.type}-${event.detail ?? ""
                  }`}
              >
                <div
                  className={`text-lg ${isHome ? "timeline-start mr-7" : "timeline-end ml-7"
                    }`}
                >
                  <div
                    className={`flex gap-5 items-center ${isHome ? "flex-row" : "flex-row-reverse"
                      }`}
                  >
